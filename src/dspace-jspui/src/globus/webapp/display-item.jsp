...
<%
    ...
    boolean admin_button = false;
    // boolean admin_button = (admin_b == null ? false : admin_b.booleanValue());

	String viewIdName = null;
	Integer viewIdValue = null;
    // get the workspace id if one has been passed
    Integer workspace_id = (Integer) request.getAttribute("workspace_id");

    if (workspace_id != null) {
        viewIdName = "workspace_id";
        viewIdValue = workspace_id;
    }

    Integer workflow_id = (Integer) request.getAttribute("workflow_id");
    if (workflow_id != null && workspace_id == null) {
        viewIdName = "workflow_id";
        viewIdValue = workflow_id;
    }
    ...
    // Url to display for citation
    String citationUrl = null;
    if (item != null) {
        citationUrl = item.getUri();
    }

    String citationString = "";
	if (authors != null && title != null && year != null && citationUrl != null) {
    	citationString = authors + "\"" + title + ",\" " + year  + ", " + citationUrl;
    }

    // The distribution license if present
    String distLicense = null;
    if (collections != null && collections.length > 0) {
        // Used provenance text for the distribution license
        distLicense = collections[0].getMetadata(Collection.COPYRIGHT_TEXT);
    }
...
%>
		...
		%>
				...
				<% if (!"".equals(distLicense)) { %>
				  <div class="panel panel-primary">
				   <div class="panel-heading"><fmt:message key="jsp.display-item.dist-license-header"/></div>
				<div class="panel-body"><%= distLicense %></div>
				</div>
				<% } %>
                <%-- <strong>Please use this identifier to cite or link to this item:
                <code><%= HandleManager.getCanonicalForm(handle) %></code></strong>--%>
                <div class="row"><div class="col-md-9"><fmt:message key="jsp.display-item.identifier"/></div></div>
                <div class="well"><!--<fmt:message key="jsp.display-item.identifier"/>-->
                	<div class="row">
	                	<div class="col-md-8 h4">
	                		...
	                		<div class="citation_string"><%= citationString %></div>
	                	</div>
...
<%
    ...
    boolean linkToDataset = ("workspace_id".equals(viewIdName) || handle != null);

%>

    <dspace:item-preview item="<%= item %>" />
    <dspace:item item="<%= item %>" collections="<%= collections %>" style="<%= displayStyle %>"
      linkToDataset="<%=linkToDataset %>"/>
<div class="container row">
<%
    String locationLink = "";
	if (handle != null) {
	    locationLink = request.getContextPath() + "/handle/" + handle;
	} else {
	    locationLink = request.getRequestURL().toString();
	}
...
<%
        ...
        {
%>
    <form class="col-md-4" method="post" action="<%= request.getContextPath() %>/view-workspaceitem">
        <input type="hidden" name="<%= viewIdName %>" value="<%= viewIdValue.intValue() %>" />
...
%>
	<a href="<%=request.getContextPath() %><%= Globus.getPublishDashboardPath() %>"><span class="btn btn-primary"><fmt:message key="jsp.mydspace.general.goto-mydspace"/></span></a>
<%
    if (viewIdName != null)
    {
%>
