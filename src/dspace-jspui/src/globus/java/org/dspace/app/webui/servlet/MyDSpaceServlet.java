...
public class MyDSpaceServlet extends DSpaceServlet
{
            ...
            SQLException, AuthorizeException
    {
        ...
        try
        {
            ipsId = Integer.parseInt(request.getParameter("workspace_id"));
            inprogressSubmission = WorkspaceItem.find(context, ipsId);
        }
        ...
        if (inprogressSubmission == null) {
            try {
                ipsId = Integer.parseInt(request.getParameter("workflow_id"));
                inprogressSubmission = WorkflowItem.find(context, ipsId);
            } catch (NumberFormatException nfe) {
                inprogressSubmission = null;
            }
        }
        ...
        else if (buttonPressed.equals("submit_delete"))
        {
            ...
            if (inprogressSubmission instanceof WorkspaceItem)
            {
                log.info(LogManager.getHeader(context, "confirm_removal",
                        "workspace_item_id=" + inprogressSubmission.getID()));

                request.setAttribute("workspace.item", inprogressSubmission);
        ...
        else if (buttonPressed.equals("submit_claim"))
        {
            ...
            if (inprogressSubmission instanceof WorkflowItem)
            {
                log.info(LogManager.getHeader(context, "view_workflow",
                        "workflow_id=" + inprogressSubmission.getID()));

                request.setAttribute("workflow.item", inprogressSubmission);
                JSPManager.showJSP(request, response,
        ...
        else if (buttonPressed.equals("submit_return"))
        {
            ...
            if (inprogressSubmission instanceof WorkflowItem)
            {
                ...
                WorkflowManager.unclaim(context, (WorkflowItem) inprogressSubmission, context
                        .getCurrentUser());
            ...
            if (item != null) {
                ...
                request.setAttribute("display.all", false);
                request.setAttribute("item", item);
                request.setAttribute("collections",
                                     new Collection[] { inprogressSubmission.getCollection() });
                String item_type = (inprogressSubmission instanceof WorkspaceItem) ? "workspace_id"
                    : "workflow_id";
                request.setAttribute(item_type, inprogressSubmission.getID());
            ...
            AuthorizeException
    {
        ...
        if (buttonPressed.equals("submit_approve"))
        {
            ...
            try{
            	...
            	archived = false;
            	request.setAttribute("globus.error", e.getMessage());
                JSPManager.showJSP(request, response, "/mydspace/globus-storage-error.jsp");
            }
            ...
            if (archived){
	            ...
	            if (handle != null)
	            {
	                String displayHandle = item.getUri();
	                if (displayHandle == null) {
	                    displayHandle = HandleManager.getCanonicalForm(handle);
	                }
            ...
            AuthorizeException
    {
        ...
        if (buttonPressed.equals("submit_send"))
        {
            ...
            // changed to pass in entire collection not just handle
            ...
            // For the case where we want to reset back to the owner without a reason
            if (reason != null) {
            JSPManager
                    .showJSP(request, response, "/mydspace/task-complete.jsp");
            } else {
                response.sendRedirect(request.getContextPath() + Globus.getPublishDashboardPath());
            }
            context.complete();
        }
        else
            ...
            SQLException, AuthorizeException
    {
        ...
        if (displayMemberships) {
            memberships = Group.allMemberGroups(context, currentUser);
        }
