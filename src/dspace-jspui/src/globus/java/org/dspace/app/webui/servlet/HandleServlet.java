            ...
            SQLException, AuthorizeException
    {
        ...
        if (path != null)
        {
            ...
            try
            {
                ...
                // JCP: Update to allow multiple slashes in the path
                int lastSlash = path.lastIndexOf('/');

                if (lastSlash != firstSlash)
                {
                    // We have extra path info
                    handle = path.substring(0, lastSlash);
                    extraPathInfo = path.substring(lastSlash);
                }
                else
                {
        ...
        if (handle != null)
        {
            ...
            // Perhaps the extra path info is just part of the original
            // name that really contains multiple slashes, so look for
            // the original path just in case it exists with that name
            if (dso == null) {
                dso = HandleManager.resolveToObject(context, path);
                if (dso != null) {
                    // We found it this way, so clear the extraPathInfo so
                    // it doesn't get examined later
                    extraPathInfo = null;
                }
            }
        }
