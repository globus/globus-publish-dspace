...
public class InstallItem
{
            ...
            IOException, AuthorizeException
    {
        ...
        Collection origOwning = item.getOwningCollection();
        Collection submitCollection = is.getCollection();
        try {
            // At this point the collection for the item has not been set, but we need it
            // to know which identifier service to use, so we set it here at least on the
            // in memory object.
            item.setOwningCollection(submitCollection);
            ...
            if (origOwning != null) {
            item.setOwningCollection(origOwning);
            }
            throw new RuntimeException("Can't create an Identifier!", e);
        }

        // Now, set it the owning collection back to what it was
        item.setOwningCollection(origOwning);
        ...
        throws SQLException, IOException, AuthorizeException
    {
        ...
        for (DCValue dcv : currentDateIssued)
        {
         ...
         String provDescription = "Made available in Globus Data Publication on " + now
