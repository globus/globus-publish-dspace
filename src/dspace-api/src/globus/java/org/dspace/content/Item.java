...
public class Item extends DSpaceObject
{
    ...
    {
        // May end up setting owning collection back to null in some cases, so handle
        // that by setting the column value to null also
        if (c != null) {
            itemRow.setColumn("owning_collection", c.getID());
        } else {
            itemRow.setColumnNull("owning_collection");
        }
        modified = true;
    }
    ...
    /**
     *
     * Associate Globus endpoint with the item by storing to the 
     * Globus schema in the metadata
     *
     */
    public boolean setGlobusEndpoint(String epName){
    	log.info("Setting Globus endpoint to item " + epName);
        this.clearMetadata("globus", "shared_endpoint", "name", null);
        this.addMetadata("globus", "shared_endpoint", "name", null, epName);
    	return true;
    }

    /**
     *
     * Get associated Globus endpoint from the Item metadata
     *
     */
    public String getGlobusEndpoint(){
    	log.info("Getting Globus endpoint for item");
    	DCValue[] dc = this.getMetadata("globus", "shared_endpoint", "name", null);
    ...
    /**
     *
     * Associate Globus path in the item metadata
     *
     */
    public boolean setGlobusSharePath(String path){
        this.clearMetadata("globus", "shared_endpoint", "path", null);
        this.addMetadata("globus", "shared_endpoint", "path", null, path);
        return true;
    }

    /**
     *
     * Get associated Globus path from metadata
     *
     */
    public String getGlobusSharePath(){
        DCValue[] dc = this.getMetadata("globus", "shared_endpoint", "path", null);
        if (dc == null || dc.length < 1){
            return null;
        }
        return dc[0].value;
    ...
    {
    	log.warn("Creating bitstreams disabled " + name);
    	return null;
        /*
...
//               Remove from indices
            ...
            throws java.sql.SQLException, AuthorizeException
    {
        ...
        // removed this change of policies as we dont use bitstreams.
    ...
    {
        String uri = "";
        DCValue[] uriValue = getMetadata("dc", "identifier", "uri", Item.ANY);
        if (uriValue != null && uriValue.length > 0)
        {
            uri = uriValue[0].value;
        }
        uri = testValidURL(uri);
        if (uri == null)
        {
            Context ctx = ourContext;
            String resolver = null;
            if (ctx != null)
            {
                resolver = GlobusConfigurationManager.getProperty(ourContext,
                        HandleServerIdentifierProvider.CFG_RESOLVER, null, this);
            }
            if (resolver == null) {
                resolver = org.dspace.core.ConfigurationManager.getProperty("handle.canonical.prefix");
            }
            if (resolver == null) {
                resolver ="1234567890";
            }
            ...
            if (handle != null) {
                // See if we get a valid one from the resolver and the handle
                uri = testValidURL(resolver + handle);
            }
        }
        return uri;
    }
     ...
     * @return
     */
    private String testValidURL(String uri)
    {
        try
        {
            URL url = new URL(uri);
