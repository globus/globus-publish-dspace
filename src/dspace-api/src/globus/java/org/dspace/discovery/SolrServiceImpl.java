...
public class SolrServiceImpl implements SearchService, IndexingService {
        ...
        try
        {
            ...
            if (force)
            {
                ...
                try {
                    ...
                    log.info("Could not resolve handle " + handle);
                }
    ...
    protected boolean requiresIndexing(String handle, Date lastModified)
        ...
        SolrQuery query = new SolrQuery();
        // Globus Added
        handle = handle.replace("\"", "\\\""); // Escape any explict quote marks in the handle
        handle = "\"" + handle + "\""; // Surround handle with quotes to escape other special chars
