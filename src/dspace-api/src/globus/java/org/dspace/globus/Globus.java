...
public class Globus
{
    ...
    // JSONLD files
    private static final Properties dublinCoreDataciteMap =
        loadMetadataMap(getGlobusConfigProperty(GLOBUS_METADATA_MAPPING_PROP));

    public static Properties loadMetadataMap(String fname)
    {
        Properties props = new Properties();
        loadMetadataMap(props, fname);
        return props;
    }

    public static void loadMetadataMap(Properties map, String fname)
    {
    ...
    public static Map<String, Object> getAllMetadataAsMap(Context context,
                                                          Item item,
                                                          Collection collection)
    {
        try {
            ...
            metadataMap.put("_comment",
                            "This file was auto-generated by Globus");
            metadataMap.put(makeJSONLDKey("globus", "publication",
                                          "submitter"),
            ...
            schemas.add("globus");
            String key;
            DCValue[] dcv = item.getMetadata(Item.ANY, Item.ANY,
                                             Item.ANY, Item.ANY);
            for (DCValue dcval : dcv) {
                ...
                if (!metadataMap.containsKey(key)) {
                    ...
                    metadataMap.put(key, valueArray);
                } else {
                    valueArray = (List<Object>) metadataMap .get(key);
                }
                valueArray.add(dcval.value);
            }
            // create JSON-LD context heading
            Map<String, Object> jsonLDContext = new HashMap<>();
            MetadataSchema metadataSchema;
            for (String s : schemas) {
                metadataSchema = MetadataSchema.find(context, s);
                jsonLDContext.put(s, metadataSchema.getNamespace());
            }
            metadataMap.put("@context", jsonLDContext);
            return metadataMap;
        } catch (Exception e) {
    ...
    {
            JSONObject dataset = new JSONObject(getAllMetadataAsMap(context, item, collection));
            return dataset;
    }
    ...
    private static void getSubGroupsTransitive(Group g, Set<Group> groups)
    {
     ...
     * @return
     */
    public static String[] visibleToListForDSpaceObject(Context context,
                                                        DSpaceObject dso,
                                                        String groupUUIDPrefix)
    {
        try
        {
            if (groupUUIDPrefix == null) {
                groupUUIDPrefix = "urn:globus:groups:id:";
            }

            Group[] authorizedGroups = null;
            do {
                authorizedGroups = AuthorizeManager.getAuthorizedGroups(context, dso, 
                                                                        Constants.READ);
                dso = dso.getParentObject();
            } while ((authorizedGroups == null || authorizedGroups.length == 0) && dso != null );
        ...
        {
            logger.error("Unable to get groups " + e);
        }
        return null;
    }
