...
//        getInputsReader();
            ...
            AuthorizeException
    {
        ...
        // if we're finished with the describe step (or have come back to edit it)
        // create the artifact file
        if ((currentPage == 1 && buttonPressed.equals(PREVIOUS_BUTTON))
                || (buttonPressed.startsWith(PROGRESS_BAR_PREFIX))
                || (currentPage >=  getNumberOfPages(request, subInfo) && buttonPressed.equals(NEXT_BUTTON)))
        {
        	// If we're finished with the Metadata create the Artifact file
            // Important to do it here, if we do it in post processing or summary preprocessing
            // Then a refresh of the summary page will start a new artifact transfer which wont
            // allow the user to complete the page. If we do it in the transfer step then users
            // can skip it by using the top level navigation and going between describe and summary
            InProgressSubmission subItem = subInfo.getSubmissionItem();
            GlobusStorageManagement.createMetadataFile(context, Globus.getPrivlegedClient(), subItem.getItem(), subItem.getCollection());
        }
            ...
            SubmissionInfo subInfo) throws ServletException
    {
        ...
        {
            coll = subInfo.getSubmissionItem().getCollection();
            collectionHandle = coll.getHandle();
        }

        // get number of input pages (i.e. "Describe" pages)
        try
        {
            return getInputsReader(coll).getNumberInputPages(collectionHandle);
        }
        catch (DCInputsReaderException e)
        {
            throw new ServletException(e);
    ...
    /**
     * Get an inputs reader appropriate for a given collection
     * @param coll The collection to read from. null is ok in which case you get a default reader
     * @return
     * @throws ServletException
     */
    public static DCInputsReader getInputsReader(Collection coll) throws ServletException
    {
        if (coll != null) {
            return getInputsReader(coll.getFormFile());
        } else {
            try
            {
