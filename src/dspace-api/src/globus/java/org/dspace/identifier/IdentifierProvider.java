...
public abstract class IdentifierProvider {
    ...
    /** Every identifier configuration stored needs to have a property with this identifier so that
     * it can be discovered later.
     */
    public static final String IDENTIFIER_CONFIGURATION_CFG_NAME = "identifier.config.name";

    protected static final ConfigurableProperty IdentifierConfigProp = new ConfigurableProperty(
            IDENTIFIER_CONFIGURATION_CFG_NAME, "New Configuration Name", null,
            DataType.STRING, "Logical Name for this Configuration", true);

    protected IdentifierService parentService;

    protected ConfigurationService configurationService;

    protected boolean systemDefaultProvider = false;
    ...
    public void setSystemDefaultProvider(boolean systemDefault) {
        this.systemDefaultProvider = systemDefault;
    }
    ...
    protected static String getConfigProperty(Context context, String configId, DSpaceObject dso)
    {
        String configGroup = GlobusIdentifierService.getIdentifierConfigNameForDso(context, dso);
        return GlobusConfigurationManager.getProperty(context, configId, configGroup, dso);
    }

    protected static Object getConfigProperty(Context context, DSpaceObject dso, ConfigurableProperty property)
    {
        String configGroup = GlobusIdentifierService.getIdentifierConfigNameForDso(context, dso);
        return GlobusConfigurationManager.getProperty(context, dso, property, configGroup);
    }
